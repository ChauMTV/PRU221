...
/// </summary>
public class PointSpawnerInspect : MonoBehaviour
{
    [HideInInspector]
    ...
    private PointSpawner spawnPoint;
    ...
    void OnEnable()
    {
        spawnPoint = GetComponent<PointSpawner>();
        Debug.Assert(spawnPoint, "Wrong stuff settings");
        // Initiate waves list
        enemies.Clear();
        foreach (PointSpawner.Wave wave in spawnPoint.waves)
        {
    ...
    public void UpdateWaveList()
    {
        ...
        while (spawnPoint.waves.Count < enemies.Count)
        {
            spawnPoint.waves.Add(new PointSpawner.Wave());
        }
